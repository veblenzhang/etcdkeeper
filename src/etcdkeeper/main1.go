...
var (
	...
	useAuthV3     = flag.Bool("authv3", true, "use auth")
...
func main() {
	...
	//http.Handle("/", http.FileServer(http.Dir("E://etcdkeeper-master//assets"))) // view static directory
...
func connectV2(w http.ResponseWriter, r *http.Request) {
	...
	log.Println("GET", "v2", host)

	if *useAuth {
		log.Println("root", "v2", host)
		...
		for _, p := range permissions {
			...
			if resp, err := kapi.Get(context.Background(), pKey, opt); err != nil {
				...
				if resp.Node == nil {
					...
					log.Println("GET", resp.Node, all , min ,max)
...
func connect(w http.ResponseWriter, r *http.Request) {
	...
	if uinfo, ok := sess.Get("uinfo").(*userInfo); ok {
		if host == uinfo.host && uname == uinfo.uname && passwd == uinfo.passwd {
			//info := getInfo(host)
			//b, _ := json.Marshal(map[string]interface{}{"status":"running", "info":info})
			b, _ := json.Marshal(map[string]interface{}{"status":"running"})
			io.WriteString(w, string(b))
			return
		}
	}
...
func getPath(w http.ResponseWriter, r *http.Request) {
	...
	if v != nil {
		...
		if originKey != separator {
			...
			presp, err = cli.Get(ctx, originKey)
			if err != nil {
				data["errorCode"] = 500
				data["message"] = err.Error()
				dataByte, _ := json.Marshal(data)
		...
		for _, p := range permissions {
			...
			if rangeEnd != "" {
				...
				resp, err = cli.Get(ctx, key, clientv3.WithSort(clientv3.SortByKey, clientv3.SortAscend))
			}
			if err != nil {
				data["errorCode"] = 500
				data["message"] = err.Error()
			...
			if len(resp.Kvs) > 20000 {
				data["errorCode"] = 500
				data["message"] = "数据太多了，请输入更具体的key"
				dataByte, _ := json.Marshal(data)
				io.WriteString(w, string(dataByte))
				return
			}
...
func del(w http.ResponseWriter, r *http.Request) {
	...
	if dir == "true" {
		if _, err := cli.Delete(ctx, key +separator, clientv3.WithPrefix());err != nil {
			io.WriteString(w, err.Error())
			return
		}
	}
...
func getTTL(cli *clientv3.Client, lease int64) int64 {
	ctx, _ := context.WithTimeout(context.Background(), 1 * time.Second)
	resp, err := cli.Lease.TimeToLive(ctx, clientv3.LeaseID(lease))
	if err != nil {
		return 0
	}
	if resp.TTL == -1 {
...
func getPermissionPrefix(host, uname, key string) ([][]string, error) {
	if !*useAuth {
		...
		if resp, err := rootCli.UserList(ctx); err != nil {
			return nil, err
		} else {
			// Find user permissions
			set := make(map[string]string)
			for _, u := range resp.Users {
				if u == uname {
					ur, err := rootCli.UserGet(ctx, u)
					if err != nil {
						return nil, err
					}
					for _, r := range ur.Roles {
						rr, err := rootCli.RoleGet(ctx, r)
						if err != nil {
							return nil, err
						}
